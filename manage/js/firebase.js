    ...
    try {
        ...
        if (!querySnapshot.empty) {
            querySnapshot.forEach(doc => {
                ...
                data.docID = doc.id;
                container.innerHTML += `
                <div class="card p-2 bg-dark text-white" id="projectContainer-${data.docID}">
                    <img class="img-fluid" width="200" src="${data.projectImageAddress || defaultImageAddress}" alt="${data.projectName}" />
                    <h5 class="my-3">${data.projectName}</h5>
            ...
            const cards = container.querySelectorAll('.btnDeletProject, .btnEditProject');
            cards.forEach((card) => {
            ...
            <div class="card bg-dark" id="certificate-${data.certificateName}">
                <div class="card-body text-white">
                    ...
                    <div class="d-flex flex-wrap gap-2 justify-content-end gap-2 mt-3">
                        <a class="btn btn-link text-white me-auto "
...
function showData(data, docID) {
    ...
    textImage.value = data.textImage
